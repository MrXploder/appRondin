...
if($postdata){
	...
	try{
		...
		if(isset($founded)){
			$adminData = DB::queryFirstRow("SELECT * FROM `operadores` WHERE `superadmin_pass` = %s", $request["superadmin_pass"]);
			$payLoad["status"] 			 = "found";
			$payLoad["admin_token"]  = $adminData["admin_token"];
			$payLoad["access_level"] = $adminData["access_level"];
